...
const classService = {
    ...
    async downloadBulkTemplate() {
        const response = await api.get('/classes/bulk-download-template', {
            responseType: 'blob'
        })
        const url = window.URL.createObjectURL(new Blob([response.data]))
        const link = document.createElement('a')
        link.href = url
        link.setAttribute('download', 'Bulk_Student_Class_Import_Template.xlsx')
        document.body.appendChild(link)
        link.click()
        link.remove()
    },

    importStudentsToClasses: async (file: File): Promise<any> => {
        const formData = new FormData();
        formData.append('file', file);
        const response = await api.post(`/classes/bulk-import-students`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        });
